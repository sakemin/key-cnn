...
class Tonal_Fragment(object):
    ...
    def get_key_softmax(self):
        # Apply softmax to key correlations
        key_values = np.array(list(self.key_dict.values())) * 5
        exp_values = np.exp(key_values - np.max(key_values))  # Subtract max for numerical stability
        softmax_values = exp_values / np.sum(exp_values)
        key_softmax = dict(zip(self.key_dict.keys(), softmax_values))
        return key_softmax
